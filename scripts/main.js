...
function readQuote(day) {
    db.collection("quote").doc(day)                                                         //name of the collection and documents should matach excatly with what you have in Firestore
...
function displayCardsDynamically(collection) {
    ...
    db.collection(collection)  //the collection called "hikes"
        .orderBy("length") // sort by length
        .limit(3)         // limit to 3 hikes
        .get()
        .then(allHikes=> {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                //chechks if the hike is already saved in the bookmarks
                // go to the current user's document in Firestore and check if the bookmarks array includes the hike's docID
                // if it does, change the icon to the filled bookmark
                currentUser.get().then(userDoc => {
                    //get the user name
